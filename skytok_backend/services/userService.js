...
//     console.log(
//       "WARNING: Redis is running in TEST mode! - TOKEN for user 1 is setted!"
//     );
//     await client.set(
//       "1",
//       "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiMSIsImlhdCI6MTY0NjI0Njk1OSwiZXhwIjoxNjQ4ODM4OTU5fQ.4_81XxmUJlum6HuD3qCAa6b46mcvjo7EhE2Dh12dRog"
//     );
//   }
// }
...
let UserService = {
  register: async function (req, res) {
    ...
    if (
      ...
      console.log(
        `Username ${username} or password ${password} or mail ${mail} or firstname ${firstname} or lastname ${lastname} or threeTags ${threeTags} or language ${language} is missing!`
      );
      sendResponse.error(res, "Missing parameters!");
      return;
    }
    let userExists = await db.queryPG(
    ...
    let user = await db.queryPG(
      `INSERT INTO "users" (username, password, mail, firstname, lastname, language_id) VALUES ('${username}', '${hashedPassword}', '${mail}', '${firstname}', '${lastname}', '${language}') RETURNING *`
    );
    threeTags = JSON.parse(threeTags);
    for (let i = 0; i < threeTags.length; i++) {
      let tag = threeTags[i];
      await db.queryPG(
        `INSERT INTO "users_tags_XREF" (user_id, tag_id, worth, date, time) VALUES (${user[0].id}, ${tag}, 10, CURRENT_DATE, CURRENT_TIME)`
      );
    }
  ...
  login: async function (req, res) {
    ...
    let user = await db.queryPG(
      `SELECT * from "users" WHERE username = '${usernameOrEmail}' OR mail = '${usernameOrEmail}'`
    );
    if (user.length == 0) {
  ...
  info: async function (req, res) {
    ...
    if (!for_user_name || for_user_name == "") {
      ...
      var user = await db.queryPG(
        `SELECT id, username, "biography", "picturePath" FROM "users" WHERE id = ${for_user_id}`
      );
      userFollowing = null;
    } else {
      var user = await db.queryPG(
        `SELECT id, username, "biography", "picturePath" FROM "users" WHERE username = '${for_user_name}'`
      );
      // console.log("From: " + user_id + " To: " + user[0].id);
      var userFollowing = await db.queryPG(
        `SELECT * FROM "following" WHERE "from_user_id" = ${user_id} AND "to_user_id" = ${user[0].id}`
      );
    ...
    let followers = await db.queryPG(
      `SELECT "from_user_id", users.username, "picturePath" FROM following INNER JOIN users ON users.id = "from_user_id" WHERE "to_user_id" = ${currUserId}`
    );
    let followersObj = {
      count: followers.length,
      followers: followers,
    };
    let following = await db.queryPG(
      `SELECT "to_user_id", users.username, "picturePath" FROM following INNER JOIN users ON users.id = "to_user_id" WHERE from_user_id = ${currUserId}`
    );
    let followingObj = {
      count: following.length,
      following: following,
    };
    let likeCount = await db.queryPG(
      `SELECT COUNT("videos_liked"."video_id") FROM "videos_liked" INNER JOIN videos ON videos.id = "videos_liked"."video_id" WHERE "videos"."user_id" = ${currUserId}`
    );
...
async function checkIfUserIsLive(user_id) {
  let userLive = await db.queryPG(
    `SELECT * FROM users_live WHERE user_id = ${user_id}`
  );
  if (userLive == null) {
    return false;
  }
